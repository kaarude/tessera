...
export async function PATCH(request: Request) {
  try {
    ...
    if (typeof parsed.data === "object" && parsed.data && "readAll" in parsed.data && parsed.data.readAll === true) {
      await prisma.notification.updateMany({
        where: { userId: user.id, isRead: false },
        data: { isRead: true },
      });
    } else if (typeof parsed.data === "object" && parsed.data && "id" in parsed.data) {
