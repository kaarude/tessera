...
import { withRoute, mapError, logAudit, requireAdmin } from "@/lib/route";
...
export const POST = withRoute(
  async ({ user, body }) => {
    ...
    const { email, name, password, isAdmin: makeAdmin, teamIds, roleIds } = parsed.data;
    ...
    if (teamIds.length > 0) {
      await prisma.teamMembership.createMany({
        data: teamIds.map((tid) => ({ userId: created.id, teamId: tid })),
        skipDuplicates: true,
      });
    }
    if (roleIds.length > 0) {
      await prisma.userRole.createMany({
        data: roleIds.map((rid) => ({ userId: created.id, roleId: rid })),
        skipDuplicates: true,
      });
    }
