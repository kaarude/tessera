...
export default function RolesPage() {
  ...
  function togglePermission(permission: string, current: string[], setter: (p: string[]) => void) {
  ...
  const categorizedPermissions = {
    Platform: ALL_PERMISSIONS.filter((p) => p.startsWith("users:") || p.startsWith("teams:") || p.startsWith("groups:") || p.startsWith("roles:") || p.startsWith("admin:") || p === "audit:view_all"),
    ...
    Audit: ALL_PERMISSIONS.filter((p) => p.startsWith("audit:") && p !== "audit:view_all"),
  };
        ...
        {showCreate && (
          <div className="rounded-xl border border-border bg-card p-5 shadow-lg">
            ...
            <div className="space-y-4">
              ...
              />
              <button
                onClick={() => createMutation.mutate({ ...newRole, teamId: currentTeamId })}
              ...
              <div key={role.id} className="rounded-xl border border-border bg-card p-5">
                ...
                {editingRole?.id === role.id && (
                  <div className="mt-4 space-y-4 border-t border-border pt-4">
                    {Object.entries(categorizedPermissions).map(([category, perms]) => (
                      <div key={category}>
                        ...
                        <div className="grid grid-cols-1 gap-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                          {perms.map((perm) => {
                            const isChecked = editingRole.permissions?.some((p: any) => p.permission === perm);
                            return (
                              <label
                                key={perm}
                              ...
                              >
                                <input
                                  ...
                                  checked={isChecked}
                                  onChange={() => {
                                    const current = editingRole.permissions.map((p: any) => p.permission);
                                    const updated = isChecked ? current.filter((p: string) => p !== perm) : [...current, perm];
                                    setEditingRole({ ...editingRole, permissions: updated.map((p: string) => ({ permission: p })) });
                                  }}
                    ...
                    <button
                      onClick={() => updateMutation.mutate({ id: role.id, permissions: editingRole.permissions.map((p: any) => p.permission) })}
